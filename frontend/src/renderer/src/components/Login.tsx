...
export function Login() {
  const { formik, border, setErrorLogin, displayError, loading } = LoginHooks();
  return (
              ...
              <form className="mt-8 space-y-6" onSubmit={formik.handleSubmit}>
                <div>
                  <input
                    ...
                    onChange={(e) => { formik.handleChange(e); setErrorLogin(false) }}
                    name="usuario"
                    id="usuario"
                  />
                </div>
                ...
                <div>
                  ...
                  >
                    {!loading ? "Acessar" : <div style={{borderTopColor: 'transparent'}} className="w-4 h-4 border-4 border-cyan-100 border-solid rounded-full animate-spin"></div>}
                  </button>
                </div>
                {displayError ? (<div className="text-red-500 text-base w-full text-center">Usuário ou senha inválidos</div>) : null}
